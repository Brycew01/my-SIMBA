...
void setupPollSet() {
  ...
  pollFileDescriptors =
      (struct pollfd *)sCalloc(POLL_SET_SIZE, sizeof(struct pollfd));
}
...
int pollCall(int timeInMilliSeconds) {
  ...
  // If timeInMilliSeconds == 0 it will return immediately after looking at the
  // poll set
  ...
  if ((pollValue = poll(pollFileDescriptors, maxFileDescriptor,
...
static void growPollSet(int newSetSize) {
  ...
  // newSetSize);
  pollFileDescriptors =
      srealloc(pollFileDescriptors, newSetSize * sizeof(struct pollfd));
